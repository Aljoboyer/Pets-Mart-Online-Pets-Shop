...
const PetAccessories: React.FC = () => {
    ...
    const accessorieses: any = useAppSelector((state: any) => state.petstore.allAccessories);
    ...
    const FilterHandler = (item: string) => {
        ...
        if(item === 'dog')
        {
            const dogdata = accessorieses.filter((data: any) => data.type === item);
            setFilterdata(dogdata)
        }
        if(item === 'cat')
        {
            const catdata = accessorieses.filter((data: any) => data.type === item);
            setFilterdata(catdata)
        }
        if(item === 'other')
        {
            const otherdata = accessorieses.filter((data: any) => data.type === item);
    ...
    return (
        <div className="container-fluid">
            ...
            </Row>
            {
                ...
                <Row className='justify-content-center my-4'>
                    {
                        sliceAccessories?.map((accessories: any) => <Accessories key={accessories._id} accessories={accessories}></Accessories>)
                    }
